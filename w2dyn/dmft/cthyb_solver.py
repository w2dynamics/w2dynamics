...
from w2dyn.dmft.impurity import (lattice_convention,
                                 ImpuritySolver,
    ...
    def solve_component(self,iter_no,isector,icomponent,mc_config_inout=[]):
        ...
        if int(self.config['QMC']['WormSearchEta']) == 1:
            ...
            for i_try in range(max_iter):
                steps_worm = np.array(0, dtype=np.longlong)
                steps_z = np.array(0, dtype=np.longlong)
        ...
        if isector == 17 and self.config['QMC']['WormMeasRaman'] == 1:
            result['raman-worm/{:05}'.format(icomponent)] = ctqmc.raman_worm.copy()
